...
if(exist('index','var')==0)
   index=[1 6 15 8 28 24 25 26]; % header | Accession | Peptide Sequence | Protein Mass 
end
...
for i=unique(Address)'
    ...
    Charges_count=CellTable2Double(cur_INTABLE(:,4:7));
...
%         uniq_charges(Address_seqmwmerged==j,:)
    ...
    OUTTABLE{i,9}=sum(sum(Charges_count)); %% Number of total spectra
